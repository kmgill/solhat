    ...
    F: DataSource + Send + Sync + 'static,
{
    ...
    let mut master_drizzle: BilinearDrizzle = BilinearDrizzle::new(
        ...
        match stack_algorithm {
            StackAlgorithm::Average => {
                StackAlgorithmImpl::Average(AverageStackBuffer::new(out_width, out_height, 3))
            }
            StackAlgorithm::Median => {
                StackAlgorithmImpl::Median(MedianStackBuffer::new(out_width, out_height, 3))
            }
            StackAlgorithm::Minimum => {
                StackAlgorithmImpl::Minimum(MinimumStackBuffer::new(out_width, out_height, 3))
            }
        },
    );
    ...
    //                     AverageStackBuffer::new(out_width, out_height, 3),
    //                 ),
    //                 StackAlgorithm::Median => {
    //                     StackAlgorithmImpl::Median(MedianStackBuffer::new(out_width, out_height, 3))
    //                 }
    //                 StackAlgorithm::Minimum => StackAlgorithmImpl::Minimum(
    //                     MinimumStackBuffer::new(out_width, out_height, 3),
    //                 ),
    //             },
    //         );
